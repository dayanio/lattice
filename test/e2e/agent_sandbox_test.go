...
var _ = Describe("Agent Sandbox", Ordered, func() {
	...
	It("sandbox can reach companion nginx via WireGuard overlay", func() {
		...
		// The init container sets up iptables REDIRECT to the transparent proxy
		// on port 15001. Outbound TCP from the nginx workload container is
		// transparently routed through gVisor/WireGuard — no SOCKS5 needed.
		Eventually(func() error {
			output, err := execInContainer(clientset, restConfig, testNS, podName, "nginx",
	...
	It("sandbox cannot reach non-lattice IP addresses", func() {
		podName := waitForPodRunningReady(clientset, testNS, sandboxName, "30s")
		// iptables REDIRECT sends all TCP from the nginx workload through the
		// transparent proxy. The EgressFilter (--egress-default-deny) blocks
		// non-overlay destinations and writes drop audit events for Scenario 6.
		Consistently(func() error {
			_, err := execInContainer(clientset, restConfig, testNS, podName, "nginx",
				[]string{"sh", "-c",
					`wget -q -O - --timeout=3 "http://1.2.3.4:80"`})
			if err == nil {
	...
	It("policy deny blocks sandbox from reaching companion", func() {
		...
		// Traffic from nginx workload goes through iptables REDIRECT → transparent proxy → gVisor policy engine.
		Eventually(func() error {
			_, err := execInContainer(clientset, restConfig, testNS, podName, "nginx",
				[]string{"sh", "-c", fmt.Sprintf(
					`wget -q -O - --timeout=3 "http://%s:8080"`,
					companionVPNIP)})
			if err == nil {
