...
| 2 | `lattice_workspace_tunnels` never emitted | `service/monitor.go` | Replace with `ceil(sum(lattice_peer_status == 1) / 2)` |
| 3 | `lattice_node_uptime_seconds` never emitted | `scraper_system.go` | Emit process uptime in `SystemScraper` |
...
|--------|--------|---------|
| `lattice_node_uptime_seconds` | peer_id, **network_id** | Count online nodes; gauge staleness |
...
| Panel | PromQL |
|-------|--------|
| 在线节点数 | `count(last_over_time(lattice_node_uptime_seconds{network_id="$ns"}[5m]))` |
| 实时吞吐 TX (Mbps) | `sum(irate(lattice_node_traffic_bytes_total{network_id="$ns",direction="tx"}[2m])) * 8 / 1e6` |
| 平均延迟 (ms) | `avg(lattice_peer_latency_ms{network_id="$ns"})` |
| 丢包率 (%) | `avg(lattice_peer_packet_loss_percent{network_id="$ns"})` |
...
| Panel | PromQL |
|-------|--------|
| Top nodes (24h traffic) | `topk(10, sum by (peer_id)(increase(lattice_node_traffic_bytes_total{network_id="$ns"}[24h])))` |
| Node online status | `last_over_time(lattice_peer_status{network_id="$ns"}[5m])` |
...
```
Agent (Pro)
  └─ SystemScraper    → lattice_node_cpu_usage_percent
                      → lattice_node_memory_bytes
                      → lattice_node_uptime_seconds   ← NEW
  └─ WireGuardScraper → lattice_peer_status
                      → lattice_node_traffic_bytes_total
                      → lattice_peer_traffic_bytes_total
                      → ...
  └─ ICMPScraper      → lattice_peer_latency_ms
                      → lattice_peer_packet_loss_percent
        │
...
### Backend
- [x] `internal/telemetry/scraper_system.go` — add `lattice_node_uptime_seconds`
