...
**明确不替代**：全系统覆盖、L3 网关仍然是内核 TUN 线的职责（见 §九 分层关系）。gVisor 不是更便宜的 NE，是另一层产品。
...
- **入站**：默认拒绝（无 `Listen` 即 RST）；放行 = 显式 `Listen`（等价现有「共享本地服务」发布的语义）或显式配置通配策略（§5.2）。
...
| `Socks5Server` | ✅ TCP CONNECT；UDP ASSOCIATE 无（§7 计划） |
...
目标地址由策略表决定（回环 / LAN 网段 / 拒绝），因此"中继到本机或本网络内"是同一机制的两个配置。Tailscale userspace-networking 模式即此实现，生产级先例。

TCP 回程靠四元组命中 endpoint（无路由环节）；UDP 回程靠会话表（§7）。
...
- `EgressFilter` 可在路由之上再收一道策略（CIDR 白名单 + DefaultDeny）。
...
| **P3** | Forwarder 通配 + 策略引擎（TCP + **UDP NAT 会话表**） | `tcp/udp.NewForwarder`；会话表 `(远端, 端口) → 本机 socket`，空闲超时回收；与 §5.2 策略档位一起交付；客户端「共享本地服务」同步升级 | 最重，随入站通配里程碑 |
...
### 8.1 SSH 回家（外面 gVisor 客户端 → 家里 NAS）
